...
inline Eigen::Matrix<double, 58, 1> Double2Vector58d(const double arr[58]) {
    Eigen::Matrix<double, 58, 1> vec;
    for (int i = 0; i < 58; ++i) vec(i) = arr[i];
    return vec;
}
...
inline void Vector2Double58d(const Eigen::Matrix<double, 58, 1>& vec, double arr[58]) {
    for (int i = 0; i < 58; ++i) arr[i] = vec(i);
}
...
void ctrl_wrapper(const int CONTROL_NUM, const Eigen::Vector2d& q, const Eigen::Vector2d& qdot,
   ...
   // Aux 함수 호출
    CoNAC(q_arr, r_arr, qdot_arr, rdot_arr, 
        ...
        CONTROL_NUM, rho, out_arr, Vn_arr);
